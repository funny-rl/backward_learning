...
class SPECTra_RNNAgent(nn.Module):
    ...
    def forward(self, inputs, hidden_state):
        ...
        embeddings = th.cat((observer_embedding, ally_embedding, enemy_embedding), dim=1) # (bs * n_agents, n_entities, hidden_size)

        if self.use_SAQA:
            attentive_embedding = self.entity_attention(embeddings[:, 0, :].unsqueeze(1), embeddings) # (bs * n_agents, 1, hidden_size)
        else:
        ...
        hidden_state = self.rnn(attentive_embedding, hidden_state)
        rec_att_embed = hidden_state.unsqueeze(1)   
