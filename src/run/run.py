...
def run(_run, _config, _log):
    ...
    if args.use_wandb:
        wandb_logs_direc = os.path.join(
            dirname(dirname(dirname(abspath(__file__)))), args.local_results_path, args.log_dir
        )
...
def run_sequential(args, logger):
    ...
    if args.env_args["logdir"] is not None:
        args.env_args["logdir"] = os.path.join(
            args.env_args["logdir"], args.env, args.name, args.group_name, args.unique_token
        )
...
def args_sanity_check(config, _log):
    ...
    if config["use_cuda"] and not th.cuda.is_available():
        ...
        _log.info("CUDA flag use_cuda was switched OFF automatically because no CUDA devices are available!")
