...
class MALRHelper():
    ...
    def encode(self, obs):
        ...
        left_team_obs = ally_info(left_team_relative, obs_left_team_direction, left_team_distance, \
            left_team_tired, self.n_allies, obs_ally_feature_dim)
        ...
        right_team_obs = enemy_info(obs_right_team, obs_right_team_direction, right_team_distance, \
            right_team_tired, self.n_enemies, obs_enemy_feature_dim)
        ...
        for file_path in self.data_paths:  
            ...
            for frame in tqdm(match_traces["frame"].unique(), desc="Processing frames"):
                ...
                A_actions = one_hot_action(
                    frame_traces[
                        [
                            c for c in frame_traces.columns if c.startswith("A") and c.endswith("_action")
                        ]
                    ]
                )
                if A_actions.empty:
                ...
                for idx in range(1, self.n_agents+1):
                    ...
                    obs_cat = np.hstack(
                        [np.array(obs_array[k], dtype=np.float32).flatten() for k in obs_array]
                    )
                    _obs.append(obs_cat)
                _obs = np.array(_obs)
                ...
                if np.concatenate(_data_, axis=0).shape[0] != np.concatenate(_label_, axis=0).shape[0]:
                    print(f"Data and label size match!, {np.concatenate(_data_, axis=0).shape}, {np.concatenate(_label_, axis=0).shape}")
...
def one_hot_action(actions):
    ...
    actions = actions.map(lambda x: action_dict[x])
    return actions