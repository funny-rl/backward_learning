...
class TraceAnimator:
    def __init__(
        ...
        anim_cols = ["inplay_records", "team1", "team2", "ball", "team1_action", "team2_action"]
        ...
        for p in plots.keys():
            ...
            if t >= inplay_start:
                if t <= inplay_end:
                    ...
                    patch_annots[p].set_text(str(action_traces.loc[t, [f"{p}_action"]].values[0]))
    ...
    def plot_init(self, ax: axes.Axes, trace_key: str):
        ...
        team1_action_args = traces[[c for c in traces.columns if c.startswith("A") and c.endswith("_action")]]
        team2_action_args = traces[[c for c in traces.columns if c.startswith("B") and c.endswith("_action")]]
        ...
        self.anim_args.loc[trace_key] = [inplay_records, team1_args, team2_args, ball_args, team1_action_args, team2_action_args]
    ...
    def run(self):
        ...
        def animate(t):
            for key in self.trace_dict.keys():
                ...
                team1_action = self.anim_args.at[key, "team1_action"]
                team2_action = self.anim_args.at[key, "team2_action"]

                if team1_args is not None:
                    TraceAnimator.animate_players(t, inplay_records, team1_action, *team1_args)
                if team2_args is not None:
                    TraceAnimator.animate_players(t, inplay_records, team2_action, *team2_args)
